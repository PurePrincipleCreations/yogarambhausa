...
function isDocumentKey(document: string): document is keyof typeof documents {
  return document in documents;
}

export const Route = createFileRoute("/legal/$document")({
  beforeLoad: ({ params }) => {
    if (!isDocumentKey(params.document)) throw notFound();
  },
  head: ({ params }) => {
    const document = isDocumentKey(params.document) ? documents[params.document] : undefined;
    return { meta: [
...
function LegalPage() {
  ...
  const content = isDocumentKey(document) ? documents[document] : undefined;
